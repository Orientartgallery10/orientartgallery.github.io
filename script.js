 ...
 *   - Hero canvas (animated Islamic geometric pattern)
 *   - Footer year
 *   - Order/quote form:
 *       · Client-side validation (name, email, service, message)
 *       · Honeypot bot detection
 *       · Spam timing detection (< 4 seconds = suspect)
 *       · fetch() to Flask/PythonAnywhere backend
 *       · async/await pattern
 *       · Loading state (button + dots)
 *       · Anti double-submit guard
 *       · Success state display
 *       · Error state display (inline + banner)
 *       · File upload preview
...
/* ================================================================
   CONFIGURATION — update API_BASE_URL after PythonAnywhere deploy
   ================================================================ */
const CONFIG = {
  /**
   * Your PythonAnywhere backend URL.
   * After deploying flask_app.py, set this to:
   *   'https://YOUR_USERNAME.pythonanywhere.com'
   * Leave trailing slash OFF.
   */
  API_BASE_URL: 'https://orientartgallery.pythonanywhere.com', 

  /** Minimum message length (chars) before backend accepts it. */
  MIN_MESSAGE_LENGTH: 30,

  /**
   * Minimum elapsed time (ms) between page load and form submit.
   * Submissions faster than this are treated as likely bot activity.
   * 4000ms = 4 seconds.
   */
...
/* ================================================================
   TIMING — record the exact moment the page became interactive.
   Used later to detect implausibly fast (bot) form submissions.
   ================================================================ */
const PAGE_LOAD_TIME = Date.now();

/* ================================================================
   DOM CACHE — centralised element references
   ================================================================ */
const DOM = {
...
/* ================================================================
   1. LOADER
   Hides the loading screen once the page window has loaded.
   Minimum display ensures animation plays (at least 2.8 s).
   ================================================================ */
(function initLoader() {
  ...
  function hideLoader() {
    ...
    setTimeout(() => {
      ...
      /* After CSS transition completes, remove from accessibility tree */
...
/* ================================================================
   2. CUSTOM CURSOR
   Only activated on pointer:fine devices (mouse/trackpad).
   Uses RAF loop for smooth independent tracking of dot vs ring.
   ================================================================ */
(function initCursor() {
...
/* ================================================================
   ...
   Fills a 1px bar at the top of the viewport based on scroll %.
   ================================================================ */
...
(function initNavbar() {
  ...
  /* Scroll state (adds background glass blur) */
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (error.name === 'AbortError' || error.name === 'TimeoutError') {
        ...
        userMessage = 'We encountered an issue communicating with our server. Please try again or contact us directly at hello@orientart.co.za';
         ...
         */
        userMessage = 'We could not reach our server at this time. Please try again in a moment, or email us directly at hello@orientart.co.za';
      } else {
        userMessage = `Something went wrong: ${error.message}. Please try again or contact us at hello@orientart.co.za`;
      }
